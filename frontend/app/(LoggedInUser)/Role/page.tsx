...
import { fetch_roles, delete_role } from '../../../utils/role'
...
const Page = () => {
  ...
  const deleteRole = async (e: FormEvent, user_id: string) => {
    e.preventDefault();
      ...
      <div className="bg-white shadow-md rounded-xl mx-10 my-4">
        <table className="min-w-max w-full table-auto">
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {data?.roles?.map((row: any, index: number) => (
                ...
                <td className="py-3 px-6 text-center">
                  <div className="flex item-center justify-center">
                    <div className="w-4 mr-2 transform text-red-500 hover:text-red-300 hover:scale-110">
                      <button onClick={(e) => deleteRole(e, row?.id)}><TrashIcon width={20} height={20} /></button>
                    </div>
                  </div>
                </td>
              </tr>
