...
import { fetch_floor,delete_floor } from '../../../utils/floor'
...
const Page = () => {
  ...
  useEffect(() => {
    const fetch_data = async () => {
      const data = await fetch_floor(user.token, user.user.id, { size, activePage, search });
      if (data?.status == 200) {
        console.log(data)
  ...
  const deleteFloor = async (e: FormEvent, floor_id: string) => {
    e.preventDefault();
    if (confirm("Are you sure, you want to delete?")) {
      setIsLoadingData(true);
      const data = await delete_floor(user.token, user.user.id, floor_id);
      alert(data?.message);
      setIsLoadingData(false);
    }
  }
  ...
  return (
    <div className="min-h-screen w-full">
      <div className="flex justify-between items-center mx-auto  px-4 py-3.5 sm:px-6 lg:px-8 bg-white shadow ">
        <h1 className="text-3xl font-bold tracking-tight text-gray-900">Floors</h1>
      ...
      <div className="bg-white shadow-md rounded-xl mx-10 my-4">
        <table className="min-w-max w-full table-auto">
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {data?.floors.map((row: any, index: number) => (
              <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                ...
                <td className="py-3 px-6 text-center">
                  ...
                  <div className="w-4 mr-2 transform text-red-500 hover:text-red-300 hover:scale-110">
                      <button onClick={(e) => deleteFloor(e, row?.id)}><TrashIcon width={20} height={20} /></button>
                    </div>
                  </div>
                </td>
              </tr>
