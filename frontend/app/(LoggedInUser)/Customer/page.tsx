...
const Page = () => {
  ...
  useEffect(() => {
    const fetch_data = async () => {
      const data = await fetch_customers(user.token, user.user.id, { size, activePage, search });
      if (data?.status == 200) {
        console.log(data)
        ...
        <table className="min-w-max w-full table-auto">
          <thead>
            <tr className="bg-gray-600 text-gray-200 text-sm leading-normal">
              ...
              <th className="py-3 px-6 text-left">Space</th>
              <th className="py-3 px-6 text-center">Floor</th>
              <th className="py-3 px-6 text-center">Meter</th>
          ...
          <tbody className="text-gray-600 text-sm font-light">
            {data?.customers.map((row: any, index: number) => (
