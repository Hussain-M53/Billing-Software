...
import { fetch_billingsByMonth } from '../../../../utils/billing'
...
const Page = (params: any) => {
    ...
    const next = () => {
        if (activePage < Math.ceil(data?.billingDetails?.length / size)) {
    ...
    useEffect(() => {
        const fetch_data = async () => {
            const data = await fetch_billingsByMonth(user.token, user.user.id, params.searchParams.id);
            if (data?.status == 200) {
                console.log(data)
        ...
        <div className="w-full max-h-screen">
            ...
            <div className="bg-white shadow-md rounded-xl mx-2 my-4">
                <table className="min-w-max w-full table-auto">
                    ...
                    <tbody className="text-gray-600 text-sm font-light">
                        {data?.billingDetails?.slice(startIndex, endIndex).map((row: any, index: number) => (
                            <tr key={index} className="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td className="py-2 px-2 text-center">
                                    <span>{row?.customer?.space?.floor?.name}</span>
                                </td>
                                <td className="py-2 px-2 text-center">
                                ...
                                <td className="py-2 px-2 text-center">
                                    <button className="bg-orange-500 hover:bg-transparent border hover:border-orange-500 hover:text-orange-500 text-white font-bold text-xs py-1 px-2 rounded">
                                        <Link
                                            href={{
                                                pathname: `/Billing/details/${row?.CID_web}`,
                                                query: {
            ...
            <div className="flex mx-12 items-center justify-between">
                <div className='rounded-full px-4 py-2 bg-gray-600 text-white'>
                    Showing {startIndex + 1} to {endIndex} of {data.billingDetails.length} results
                </div>
                <div className="flex gap-x-2">
                    ...
                    <button
                        className={`${activePage === Math.ceil(data?.billingDetails?.length / size) ? 'bg-gray-300 text-gray-100' : 'bg-gray-900 text-white hover:bg-gray-700'} flex items-center gap-2 py-2 px-4 rounded`}
                        onClick={next}
                        disabled={activePage === Math.ceil(data?.billingDetails?.length / size)}
                    >
                        Next
                    </button>
                </div>
