...
const getMeters = async (meter_ids) => {
    ...
    try {
        let query;
        if (allMeters){
            query = `Select m.name, m.id, h.TABLE_NAME, flr.name as floorName 
            ...
            , m.history_config_id from meters as m inner join HISTORY_CONFIG as h on h.ID = m.history_config_id
            inner join floors_web as flr on flr.id = m.floor_id
            where m.status = 1 and m.id in (${meter_ids})`
        }
    ...
    async comparisonChartData(req, res) {
        ...
        const chartData = await Promise.all(
            ...
            try {
                ...
                //             type: db.sequelize.QueryTypes.SELECT
                // });
                spResult = await db.sequelize.query('EXEC proc_getMonthlyConsumption @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @isMBTU = :isMBTU', {
                    replacements: {
            ...
            meter_tables.map(async item => {
               ...
               try {
                // note that line graph should be made from 1st minValue to last MaxValue
                    spResult = await db.sequelize.query('EXEC proc_getLineGraph @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @isMBTU = :isMBTU', {
                        replacements: {
                        HistoryTableID : item.history_config_id,
            ...
            meter_tables.map(async item => {
               ...
               try {
                    spResult = await db.sequelize.query('EXEC proc_getDayGraph @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @isMBTU = :isMBTU', {
                        ...
                        try {
                            const spMinMax = await db.sequelize.query('EXEC proc_getMinMaxValue @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @isMBTU = :isMBTU', {
                                replacements: {
                                HistoryTableID : item.history_config_id,
                                fromDate: newStartDate,
                                toDate: newEndDate,
...
//                try {
                    ...
                    let spResult={};
                    try {
                        spResult = await db.sequelize.query('EXEC proc_getHourGraph @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @isMBTU = :isMBTU', {
                        replacements: {
                        HistoryTableID : item.history_config_id,
                        fromDate: newStartDate,
                        toDate: newEndDate,
