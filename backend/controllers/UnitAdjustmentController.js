...
module.exports = {
    async performUnitAdjustment(res, fromDate, toDate, finalUnitsTonHour, diffUnitsTonHour, HistoryTableID, isMBTU, TableName){
        ...
        let spResult={};
        try {
            spResult = await db.sequelize.query('EXEC proc_AdjustUnits @HistoryTableID = :HistoryTableID, @fromDate = :fromDate, @toDate = :toDate, @units= :units, @isMBTU = :isMBTU', 
            {
                replacements: {
                    ...
                    isMBTU: isMBTU, // need results in Ton-Hour
                },
    ...
    async create(req, res) {
        let response = null;
        const {MeterId, docNo, docDate, fromDate, toDate, currentUnitsTonHour, finalUnitsTonHour, CoID, } = req.body;
        ...
        const unitAdjustment = await UnitAdjustment.create({
            MeterId: MeterId,
            docNo: docNo,
            docDate: moment(docDate).format('YYYY-MM-DD 00:00:00'),
    ...
    async unitAdjustments(req, res) {
        ...
        if (responseType == ResponseType.PAGINATED) {
            ...
            ////// check if we can sort by meters and date /////////// TODO
            // if (sortBy) {
    ...
    async unitAdjustment(req, res) {
        ...
        //console.log("unitAdjustment.........",unitAdjustment);
    ...
    async update(req, res) {
        ...
        if (lastUnitdjustment[0].id > id){
            ...
            //console.error('Error updating: Unit Adjustment(s) created after this adjustment');
        ...
        return res.status(200).json({unitAdjustment: await UnitAdjustmentResource(unitAdjustment)});
    },
    ...
    async destroy(req, res) {
        ...
        //console.log('lastUnitdjustment',lastUnitdjustment);

        if (lastUnitdjustment.length>0 && lastUnitdjustment[0].id > id){
            // do not allow update/ delete
            //console.error('Error deleting: Unit Adjustment(s) created after this adjustment');
